...
// a request variable for the content endpoints
var contentRequest = new XMLHttpRequest();
// another request variable for the comment endpoints
...
// used to keep track of where we are in the API
var iterations = 0;

// when we reach the bottom of our page, get more items
document.addEventListener("scroll", function(){
	if (document.documentElement.scrollHeight == Math.ceil(window.scrollY + window.innerHeight))
		pullDataSetHTML(iterations);
});

// this is the main function where we will get data from the API and set it into our HTML
function pullDataSetHTML(i){
	if (iterations <= 15){
		...
		contentRequest.open("GET","http://localhost:8080/https://ign-apis.herokuapp.com/content?startIndex=" + i * 20 + "&count=20",true);
		
		contentRequest.onload = function(){
			// wait until the fetch operation has been completed before moving on
			if (contentRequest.readyState === XMLHttpRequest.DONE)
				// this will give us an object we can work with that contains the entire page of data
				var data = JSON.parse(this.response);
			
			// one article for each of the "latest news" items that we are about to get
			for (let j = 0; j < 20; j++){
				var newArt = document.createElement("article");
				newArt.setAttribute("id","data" + (i * 20 + j));
			...
			for (let j = 0; j < 20; j++){
				var art = document.getElementById("data" + (i * 20 + j));
			...
			for (let j = 0; j < 20; j++){						
				// by the end of this loop, ids will have 20 terms separated with commas
				if (j == 0)
				...
				if (data.data[j].contentType == "article"){
					...
					"<a href>" + itemTitle + "</a>");
					// hide these if the video button is active
					if (tabButton[1].classList.contains("active"))
					...
					dataElem.getElementsByClassName("title-container")[0].insertAdjacentHTML("afterbegin", 
					"<a href>" + itemTitle + "</a>");
					if (tabButton[2].classList.contains("active"))
						dataElem.style.display = "none";
				...
				if (data.data[j].thumbnails.length > 0)
					...
					// insert a placeholder if something isn't right so the div will at least be sized right
					dataElem.getElementsByClassName("thumbnail-container")[0].insertAdjacentHTML("afterbegin",
						"<a href><img class=\"thumbnail\" src=\"images/placeholder.png\" alt=\"" + itemTitle + "\"></a>");
			...
			commentRequest.open("GET","http://localhost:8080/https://ign-apis.herokuapp.com/comments?ids=" + ids,true);
			commentRequest.onload = function(){
				...
				for (let j = 0; j < 20; j++){
					...
					if (alsoData.content[j].count == 0)
						document.getElementById("data" + (i * 20 + j)).getElementsByClassName("time-comm-container")[0]
						.insertAdjacentHTML("beforeend", "•<img class=\"comment-icon\" src=\"icons/chat-bubble-icon.png\" alt=\"comment icon\">" + 
						"<span class=\"comments\"></span>");
					else
						document.getElementById("data" + (i * 20 + j)).getElementsByClassName("time-comm-container")[0]
						.insertAdjacentHTML("beforeend", "•<img class=\"comment-icon\" src=\"icons/chat-bubble-icon.png\" alt=\"comment icon\">" + 
						"<span class=\"comments\">" + alsoData.content[j].count + "</span>");
				}	
			}
			commentRequest.send();
		}
